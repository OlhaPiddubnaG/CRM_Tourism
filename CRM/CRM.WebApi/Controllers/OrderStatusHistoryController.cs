...
public class OrderStatusHistoryController : ControllerBase
{
    ...
    {
        var response = await _sender.Send(new GetAllRequest<OrderStatusHistoryResponse>(), token);
    ...
    [ProducesResponseType(typeof(BadResponseResult), StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Create([FromBody] CreateOrderStatusHistoryCommand request, CancellationToken token)
    {
    ...
    {
        var response = await _sender.Send(new DeleteCommand<OrderStatusHistory>(id), token);
    ...
    [ProducesResponseType(typeof(ResultBaseResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(BadResponseResult), StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(UpdateOrderStatusHistoryCommand request, CancellationToken token)
    {
        var response = await _sender.Send(request, token);
