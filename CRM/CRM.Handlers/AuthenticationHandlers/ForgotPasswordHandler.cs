...
public class ForgotPasswordHandler : IRequestHandler<ForgotPasswordCommand, ResultBaseResponse>
{
    ...
    public async Task<ResultBaseResponse> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        var resetLink = $"{domain["Domain"]}/ResetPassword?token={user.PasswordResetToken}";
        ...
        return new ResultBaseResponse { Success = true, Message = "Password reset email sent successfully." };
    }
}