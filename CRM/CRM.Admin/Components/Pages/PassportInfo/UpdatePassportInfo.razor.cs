...
public partial class UpdatePassportInfo
{
    ...
    {
        var result = await PassportInfoRequest.UpdateAsync(_passportInternalDto);
        if (result)
        {
        ...
        {
            Snackbar.Add($"Помилка внесення змін до внутрішнього паспорту", Severity.Error);
        }
    }

    private async Task SubmitInternational()
    {
        var result = await PassportInfoRequest.UpdateAsync(_passportInternationalDto);
        if (result)
        {
        ...
        {
            Snackbar.Add($"Помилка внесення змін до закордонного паспорту", Severity.Error);
        }
    }
